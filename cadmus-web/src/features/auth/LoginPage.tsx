...
export const LoginPage = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 2. Derive & set the Vault Secret locally (Transient)
      // We use the raw password as the basis for PBKDF2 derivation in the crypto module
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-base font-ui overflow-hidden p-6">
      ...
      <div className="w-full max-w-md bg-surface border-2 border-border shadow-hard relative z-10 animate-in fade-in zoom-in-95 duration-500">
        {/* Header Ribbon */}
        ...
        <div className="p-12">
            <div className="flex flex-col gap-4 mb-12">
                <div className="flex items-center gap-4">
                    ...
                    <h1 className="text-3xl font-black uppercase tracking-tighter text-text leading-none pt-1">
                        Sovereign<br/><span className="text-accent opacity-80">Access</span>
                    </h1>
                </div>
                <p className="text-[10px] font-black text-subtext uppercase tracking-[0.3em] opacity-60">Authentication::Protocol::Required</p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-8">
              <div className="space-y-2">
                <label className="text-[10px] font-black text-subtext uppercase tracking-[0.2em]">Operator_Identifier</label>
                <input 
                    autoFocus
                    type="text" 
                    value={username} 
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-black text-subtext uppercase tracking-[0.2em]">Private_Access_Key</label>
                <input 
                    type="password" 
                    value={password} 
            ...
            <div className="mt-12 pt-8 border-t-2 border-border/30 flex justify-between items-center">
                <span className="text-[9px] font-black text-subtext uppercase tracking-widest">No_Profile_Found?</span>
                <Link to="/signup" className="text-[10px] font-black text-accent hover:underline uppercase tracking-tighter flex items-center gap-2">
                    <Zap className="w-3.5 h-3.5" /> REGISTER_OPERATOR
                </Link>
            </div>
        </div>
      </div>
      
      <div className="mt-10 flex flex-col items-center gap-2 text-[9px] font-black text-subtext uppercase tracking-[0.5em] opacity-40">
        <span>Cadmus_Kernel_Runtime_v0.1.0</span>
        <span>Secure_Encryption_Chain::Enabled</span>
      </div>
    </div>
  );
};
